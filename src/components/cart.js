...
const Cart = () => {
  ...
  const [localQuantities, setLocalQuantities] = useState(() => Array(cartItems.length).fill(1));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (cartDataVal) {
        dispatch(updateCartItems(cartDataVal.cartItems));
        dispatch(updateQuantities(cartDataVal.quantities));
      }
    };
    fetchData();
  }, [dispatch]);
  ...
  useEffect(() => {
    let total = 0;
    cartItemsRedux.forEach((item, index) => {
  ...
  const dlt = (index) => {
    dispatch(DLT(cartItemsRedux[index].key));
    const newQuantities = [...localQuantities];
    newQuantities.splice(index, 1);
    setLocalQuantities(newQuantities);
    dispatch(updateQuantities(newQuantities));
    const newCartData = [...cartItemsRedux];
    newCartData.splice(index, 1);
    setcartItemsRedux(newCartData);
  };
  const putData = () => {
